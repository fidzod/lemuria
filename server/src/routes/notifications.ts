...
export const notificationsRouter = new Hono<{ Variables: AppVariables }>()
  ...
  .get('/', requireAuth, async (c) => {
    ...
    const actionUser = alias(users, 'action_user');
    ...
    const rows = await db
      .select({
        ...
        actionUserUsername: actionUser.username,
        actionUserDisplayName: actionUser.displayName,
        actionUserAccentColor: actionUser.accentColor,
        actionUserAvatarUrl: actionUser.avatarUrl,
        actionUserCreatedAt: actionUser.createdAt,
        actionUserLastSeen: actionUser.lastSeen,
        postId: posts.id,
        postParentId: posts.parentId,
      })
      .from(notifications)
      ...
      .leftJoin(actionUser, eq(notifications.actionUserId, actionUser.id))
    ...
    const mapped: AppNotification[] = rows.map((row): AppNotification => {
      ...
      if (base.type === 'post_liked' || base.type === 'comment_liked') {
        return {
          ...base,
          actionUser: {
            id: row.actionUserId!,
            username: row.actionUserUsername!,
            displayName: row.actionUserDisplayName!,
            accentColor: row.actionUserAccentColor!,
            avatarUrl: row.actionUserAvatarUrl!,
            createdAt: row.actionUserCreatedAt!,
            lastSeen: row.actionUserLastSeen!,
          },
          post: {
            id: row.postId!,
            parentId: row.postParentId!,
          }
        }
      }
